...
HTMLDocument.prototype.createComment = function() {
    //FIXME: libxmljs doesn't provide the means to do this. It also adopting nodes is problematic
...
HTMLDocument.prototype.createTextNode = function(text) {
    //FIXME: libxmljs doesn't provide the means to do this. It also adopting nodes is problematic
    var doc  = libxml.parseHtmlString('<html><body>' + text + '</body></html>'),
        body = doc.root().child(0),
        node = body.child(0);
...
HTMLElement.prototype.__defineSetter__('innerHTML', function(html) {
    //FIXME: libxmljs doesn't provide the means to do this. It also sucks at adopting nodes
...
HTMLElement.prototype.insertBefore = function(newElement, referenceElement) {
    // TODO: update path and trail caches
...
HTMLElement.prototype.compareDocumentPosition = function(other) {
    ...
    // given '/html/head/style'.compareDocumentPosition('/html/head/script')
    var commonDescendant   = this._n.get(dTrail.join('/')),      // /html/head
        thisDescendant     = commonDescendant.get(tTrail[idx]),  // /html/head/style
        otherDescendant    = commonDescendant.get(oTrail[idx]),  // /html/head/script
        descendantSiblings = commonDescendant.childNodes();      // /html/head/*
    
    for (var i = 0; i < descendantSiblings.length; i++) {
        var sibling = descendantSiblings[i];
        
        if (sibling === thisDescendant) {
            return order.DOCUMENT_POSITION_FOLLOWING;
        }
        if (sibling === otherDescendant) {
